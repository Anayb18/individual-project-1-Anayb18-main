...
const getQuestionsByFilter = async (req: FindQuestionRequest, res: Response): Promise<void> => {
  ...
  const {asked_by} = req.query;

  try {
    const qlist: Question[] = await getQuestionsByOrder(order);
    const asked_byqlist: Question[] = await filterQuestionsByAskedBy(qlist, asked_by);
    const resqlist: Question[] = await filterQuestionsBySearch(asked_byqlist, search);
    res.json(resqlist);
  } catch (err: unknown) {

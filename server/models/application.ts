...
export const getQuestionsByOrder = async (order: OrderType): Promise<Question[]> => {
  try {
    ...
    // Default sorting by newest if no specific order type is matched
...
export const getTags = async (tags: Tag[]): Promise<Tag[]> => {
  try {
    // Step 1: Remove duplicate tags based on the tag name
    const uniqueTags = Array.from(new Map(tags.map(tag => [tag.name, tag])).values());

    // Step 2: Check the database for existing tags
    const existingTags = await TagModel.find({ name: { $in: uniqueTags.map(tag => tag.name) } });

    // Create a Set of existing tag names for easier lookup
    const existingTagNames = new Set(existingTags.map(tag => tag.name));

    // Step 3: Create new tags if they do not already exist
    ...
    // Return the modified list of tags
    return newTags.filter(tag => tag !== null) as Tag[];
  } catch (error) {
    return [];
  }
};
...
/**
 * Sorts the list of questions by the number of views in descending order.
 *
 * @param {Question[]} qlist - The list of questions to sort
 * @returns {Question[]} - The sorted list of questions by most viewed
 */
